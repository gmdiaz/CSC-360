...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        monkey.center = CGPointMake(self.view.bounds.width/2, self.view.bounds.height/2, 0)
        ...
        self.motionManager.startDeviceMotionUpdatesToQueue(NSOperationQueue()) {
            ...
            dispatch_async(dispatch_get_main_queue()) { // need to modify view on screen in main thread
                ...
                /// set current position of monkey -- will be regarded as initial position
                // 2D approach
                let currentX = self.monkey.center.x
                let currentY = self.monkey.center.y
                let currentZ = self.monkey.center.z
                // 3D approach
                let currentPosition = SCNVector3(self.monkey.center.x, self.monkey.center.y, self.monkey.center.z);
